...
namespace Vrs.Internal
{
    ...
    {
        private bool usePreRender = false;
        ...
        void OnPostRender()
        {
            ...
            {
                cam.targetTexture.GetNativeTexturePtr(); // Без этого почему-то при старте будут появляться помехи со случайными текстурами
            }
        }
                                              ...
                                              float parx = 0, float pary = 0)
        {
            ...
            if (VrsViewer.USE_DTR && VrsGlobal.supportDtr && Application.platform == RuntimePlatform.Android)
            {
        ...
        {
            if (usePreRender && GraphicsSettings.renderPipelineAsset != null)
            {
